...
function generateBomb() {
  let bombX, bombY;
  const snakeHead = snake[0]; // ראש הנחש
  do {
    ...
    isCollisionWithFood({ x: bombX, y: bombY }) ||
    !isWithinRadius(snakeHead, { x: bombX, y: bombY }, 3 * box) // בדיקה אם הפצצה ברדיוס של 3 קוביות מראש הנחש
  );
  return { x: bombX, y: bombY };
}

// פונקציה לבדיקה אם פצצה נמצאת ברדיוס מסוים מראש הנחש
